...
export const createPayPalOrder = functions
  ...
  try {
    ...
    const paymentData = {
      ...
      student_id: payment_id, // Using payment_id as identifier
...
export const confirmPaymentServer = functions.https.onCall(async (data, context) => {
  debugLog('confirmPaymentServer', 'Function called', { userId: context.auth?.uid, data });
...
export const createVerificationTokenServer = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'POST');
  res.set('Access-Control-Allow-Headers', 'Content-Type');
...
export const verifyEmailServer = functions.https.onCall(async (data, context) => {
  const { token } = data;
