...
export const updateUserXP = functions.https.onCall({
  timeoutSeconds: 30,
}, async (request: CallableRequest<XPUpdateData>) => {
  ...
  try {
    ...
    if (currentProgress.exists) {
      const data = currentProgress.data() as UserProgress;
...
export const awardXPForAction = functions.https.onCall({
  timeoutSeconds: 30,
}, async (request: CallableRequest<{ userId: string; action: string }>) => {
  const { data } = request;
  const { userId, action } = data;
  
  // Define XP rewards for different actions
  const xpRewards: { [key: string]: { xp: number; reason: string } } = {
...
export const getUserProgress = functions.https.onCall({
  timeoutSeconds: 30,
